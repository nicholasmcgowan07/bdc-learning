...
const mkQ=()=>({id:uid(),text:"",correct:true,rationale:""});
...
const mkTF=()=>({
  ...
  // Designer note: the most effective T/F statements are true-but-counterintuitive
  // or false-but-credible. Obvious statements teach nothing.
...
async function aiGen(desc,ctx,count){
  ...
  const n=count||1;
  const sys=`You are an expert instructional designer. Create a True or False activity with exactly ${n} statement${n===1?"":"s"}.
Rules:
- Exactly ${n} statement${n===1?"":"s"}
- Each statement is unambiguously true OR false
- The most effective statements are: true-but-counterintuitive, or false-but-credible. Avoid obvious statements.
- Mix true and false answers — don't make them all one or the other
- rationale: 1-2 sentences explaining WHY the statement is true or false. This is the learning moment.
- Statements should be concise (max 25 words) and test understanding, not recall of trivial facts
  ...
  "questions": [
    {
      ...
      "correct": true,
      "rationale": "Why this is true or false — the explanation that makes it educational"
    }
  ],
  "submitLabel": "See my results"
}`;
...
function buildTFJS(id){
  ...
  function render(){
    ...
    C.questions.forEach(function(q,qi){
      var sel=answers[q.id],locked=sel!==undefined,cor=locked&&(sel===q.correct);
      var leftBorder=locked?(cor?'#0F6E56':'#E8192C'):'#18303F';
      h+='<div style="padding:1.25rem;margin-bottom:.875rem;background:#fff;border:1.5px solid #D8DDE3;border-left:3px solid '+leftBorder+';border-radius:10px">';
      h+='<div style="display:flex;align-items:flex-start;gap:10px;margin-bottom:1rem">';
      ...
      // True / False buttons
      h+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:.5rem;margin-bottom:'+(locked?'.75rem':'0')+'">';
      [true,false].forEach(function(val){
        var label=val?'True':'False';
        var isSelected=locked&&sel===val;
        var isCorrect=val===q.correct;
        var bg='#F4F6F7',border='#D8DDE3',color='#18303F',fw=500;
        if(locked){
          if(isCorrect){bg='#E1F5EE';border='#0F6E56';color='#0F6E56';fw=700;}
          if(isSelected&&!isCorrect){bg='#FDEAEA';border='#E8192C';color='#E8192C';fw=700;}
        }
        var cursor=locked?'default':'pointer';
        h+='<div '+(locked?'':' onclick="window[\\'tf_${id}_pick\\'](\\''+ q.id +'\\','+(val?'true':'false')+')"')
          +' style="display:flex;align-items:center;justify-content:center;gap:7px;padding:.7rem 1rem;border-radius:50px;border:1.5px solid '+border+';background:'+bg+';cursor:'+cursor+';user-select:none;transition:all .12s"'
          +(locked?'':' onmouseover="this.style.borderColor=\\'#18303F\\';this.style.background=\\'#E8ECEE\\';" onmouseout="this.style.borderColor=\\'#D8DDE3\\';this.style.background=\\'#F4F6F7\\';"')
          +'>';
        if(locked&&isCorrect)h+='<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M2 7l3.5 3.5L12 3" stroke="'+color+'" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>';
        if(locked&&isSelected&&!isCorrect)h+='<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M3 3l8 8M11 3l-8 8" stroke="'+color+'" stroke-width="1.8" stroke-linecap="round"/></svg>';
        h+='<span style="font-size:15px;font-weight:'+fw+';color:'+color+'">'+label+'</span></div>';
      });
      h+='</div>';
      // Rationale
      if(locked){
        h+='<div style="padding:.625rem .875rem;border-radius:8px;background:'+(cor?'#E1F5EE':'#FAEEDA')+';border:1px solid '+(cor?'#0F6E56':'#FAC775')+'">';
        h+='<span style="font-size:13px;font-weight:600;color:'+(cor?'#0F6E56':'#854F0B')+'">'+(cor?'Correct — ':'Incorrect — ')+ec(q.rationale)+'</span>';
        h+='</div>';
      }
      h+='</div>';
    });
  ...
  function showResults(){
    ...
    C.questions.forEach(function(q,qi){
      ...
      h+='<div style="font-size:15px;font-weight:500;color:#18303F;line-height:1.4;margin-bottom:.375rem">'+ec(q.text)+'</div>';
      if(!cor)h+='<div style="font-size:13px;color:#854F0B;margin-bottom:.25rem">You said: <strong>'+(sel?'True':'False')+'</strong> — Correct answer: <strong>'+(q.correct?'True':'False')+'</strong></div>';
      h+='<div style="font-size:13px;color:#18303F;line-height:1.55">'+ec(q.rationale)+'</div>';
      h+='</div>';
    });
...
<div id="${id}">
  <div style="margin-bottom:1.75rem">
    <div style="font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.14em;color:#E8192C;margin-bottom:.5rem">True or False</div>
...
<title>${eH(title||'True or False Activity')}</title>
...
// ─── React preview ──────────────────────────────────────────────────────────
function TFPreview({data}){
  const [answers,setAnswers]=useState({});
  const [showResults,setShowResults]=useState(false);
  const allAnswered=data.questions.every(q=>answers[q.id]!==undefined);
  const pick=(qid,val)=>{if(answers[qid]!==undefined)return;setAnswers(a=>({...a,[qid]:val}));};
    ...
    <div>
      ...
      {data.questions.map((q,qi)=>{
        ...
        return(<div key={q.id} style={{padding:".875rem 1rem",marginBottom:".5rem",borderRadius:8,border:`1px solid ${cor?GRN:"#FAC775"}`,background:cor?GRN_L:AMB_L}}>
          ...
          <div style={{fontSize:15,fontWeight:500,color:NAVY,lineHeight:1.4,marginBottom:".375rem"}}>{q.text||<em style={{opacity:.4}}>Empty statement</em>}</div>
          {!cor&&<div style={{fontSize:13,color:AMB,marginBottom:".25rem"}}>You said: <strong>{sel?"True":"False"}</strong> — Correct answer: <strong>{q.correct?"True":"False"}</strong></div>}
          <div style={{fontSize:13,color:TS,lineHeight:1.55}}>{q.rationale}</div>
        </div>);
      })}
  ...
  return(<div>
    {data.questions.map((q,qi)=>{
      const sel=answers[q.id];const locked=sel!==undefined;const cor=locked&&sel===q.correct;
      return(<div key={q.id} style={{padding:"1.25rem",marginBottom:".875rem",background:"#fff",border:`1.5px solid ${BORDER}`,borderLeft:`3px solid ${locked?cor?GRN:RED:NAVY}`,borderRadius:10}}>
        <div style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:"1rem"}}>
          ...
          <div style={{fontSize:16,fontWeight:500,color:NAVY,lineHeight:1.5}}>{q.text||<em style={{color:TT,fontWeight:400}}>Empty statement</em>}</div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:".5rem",marginBottom:locked?".75rem":0}}>
          {[true,false].map(val=>{
            const label=val?"True":"False";
            const isSelected=locked&&sel===val;
            const isCorrect=val===q.correct;
            let bg="#F4F6F7",border=BORDER,color=NAVY,fw=500;
            if(locked){
              if(isCorrect){bg=GRN_L;border=GRN;color=GRN;fw=700;}
              if(isSelected&&!isCorrect){bg=RED_L;border=RED;color=RED;fw=700;}
            }
            return(<button key={String(val)} onClick={()=>pick(q.id,val)} disabled={locked} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:7,padding:".7rem 1rem",borderRadius:50,border:`1.5px solid ${border}`,background:bg,cursor:locked?"default":"pointer",transition:"all .12s",fontFamily:SANS}}>
              {locked&&isCorrect&&<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M2 7l3.5 3.5L12 3" stroke={color} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>}
              {locked&&isSelected&&!isCorrect&&<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M3 3l8 8M11 3l-8 8" stroke={color} strokeWidth="1.8" strokeLinecap="round"/></svg>}
              <span style={{fontSize:15,fontWeight:fw,color}}>{label}</span>
            </button>);
          })}
        </div>
        {locked&&<div style={{padding:".625rem .875rem",borderRadius:8,background:cor?GRN_L:AMB_L,border:`1px solid ${cor?GRN:"#FAC775"}`}}>
          <span style={{fontSize:13,fontWeight:600,color:cor?GRN:AMB}}>{cor?"Correct — ":"Incorrect — "}</span>
          <span style={{fontSize:13,color:TS}}>{q.rationale||<em style={{opacity:.5}}>No rationale yet</em>}</span>
        </div>}
      </div>);
    })}
    <div style={{display:"flex",justifyContent:"flex-end",marginTop:".5rem"}}>
      <button onClick={()=>allAnswered&&setShowResults(true)} disabled={!allAnswered} style={{padding:".6rem 1.5rem",borderRadius:50,border:"none",background:allAnswered?RED:BORDER,color:allAnswered?"#fff":TT,fontFamily:SANS,fontSize:15,fontWeight:700,cursor:allAnswered?"pointer":"default",transition:"all .2s"}}>
        {allAnswered?data.submitLabel:`Answer ${data.questions.filter(q=>answers[q.id]===undefined).length} more statement${data.questions.filter(q=>answers[q.id]===undefined).length===1?"":"s"} first`}
      </button>
    </div>
  </div>);
}
...
function TFEditor({content,onChange}){
  ...
  return(<div>
    <div style={{marginBottom:".875rem"}}><Lbl>Topic *</Lbl><input value={content.topic} onChange={e=>set("topic",e.target.value)} placeholder="e.g. Common myths about data protection" style={iStyle}/></div>
    <div style={{marginBottom:"1.25rem"}}><Lbl>Objective *</Lbl><input value={content.objective} onChange={e=>set("objective",e.target.value)} placeholder="e.g. Distinguish fact from common misconception in data handling." style={iStyle}/></div>
    <div style={{marginBottom:".375rem"}}><Lbl>Statements ({content.questions.length})</Lbl></div>
    <div style={{fontSize:14,color:TS,marginBottom:".875rem",lineHeight:1.6}}>The most effective statements are true-but-counterintuitive or false-but-credible. Avoid obvious ones — they teach nothing.</div>

    {content.questions.map((q,qi)=>(
      <div key={q.id} style={{padding:".875rem",marginBottom:".5rem",background:"#F8F9FB",border:`1px solid ${BORDER}`,borderLeft:`3px solid ${q.correct?GRN:RED}`,borderRadius:10}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:".625rem"}}>
          <span style={{fontSize:13,fontWeight:700,color:NAVY,flexShrink:0}}>Statement {qi+1}</span>
          <div style={{flex:1}}/>
        ...
        <textarea value={q.text} onChange={e=>setQ(qi,"text",e.target.value)} placeholder="Write a statement learners must evaluate as true or false…" style={{...iStyle,resize:"none",lineHeight:1.55,marginBottom:".625rem"}} rows={2}/>

        {/* True / False toggle */}
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:".625rem"}}>
          <span style={{fontSize:13,color:TT,flexShrink:0}}>This statement is:</span>
          <div style={{display:"flex",gap:".375rem"}}>
            {[true,false].map(val=><button key={String(val)} onClick={()=>setQ(qi,"correct",val)} style={{padding:".35rem 1.125rem",borderRadius:50,border:`1.5px solid ${q.correct===val?(val?GRN:RED):BORDER}`,background:q.correct===val?(val?GRN_L:RED_L):"#fff",color:q.correct===val?(val?GRN:RED):TT,fontFamily:SANS,fontSize:13,fontWeight:q.correct===val?700:400,cursor:"pointer",transition:"all .15s"}}>{val?"True":"False"}</button>)}
          </div>
        </div>

        <div>
          <div style={{fontSize:11,fontWeight:600,color:q.correct?GRN:RED,marginBottom:".25rem"}}>Rationale — why is this {q.correct?"true":"false"}?</div>
          <textarea value={q.rationale} onChange={e=>setQ(qi,"rationale",e.target.value)} placeholder="Explain the reasoning. This is the learning moment — it shows after every answer." style={{...iStyle,resize:"none",lineHeight:1.55}} rows={2}/>
        </div>
      </div>
    ))}
    <button onClick={addQ} style={{width:"100%",fontSize:14,color:NAVY,background:"none",border:`1px dashed ${BORDER}`,borderRadius:8,padding:".5rem",cursor:"pointer",fontFamily:SANS,marginTop:".25rem"}}>+ Add statement</button>
  </div>);
}
...
function AIPanel({onApply,questionCount}){
  ...
  return(<div style={{marginBottom:"1.25rem"}}>
    {!open&&(<button onClick={()=>{setOpen(true);setTimeout(()=>taRef.current?.focus(),80);}} style={{width:"100%",display:"flex",alignItems:"center",gap:12,padding:"1rem 1.25rem",borderRadius:12,border:`1.5px solid ${BORDER}`,background:"#fff",cursor:"pointer",textAlign:"left",fontFamily:SANS,boxShadow:"0 1px 3px rgba(24,48,63,.04)"}} onMouseEnter={e=>{e.currentTarget.style.borderColor=NAVY;e.currentTarget.style.boxShadow="0 2px 8px rgba(24,48,63,.1)";}} onMouseLeave={e=>{e.currentTarget.style.borderColor=BORDER;e.currentTarget.style.boxShadow="0 1px 3px rgba(24,48,63,.04)";}}>
      ...
      <div style={{flex:1}}><div style={{fontSize:15,fontWeight:600,color:NAVY,marginBottom:2}}>Generate with AI</div><div style={{fontSize:14,color:TT}}>Describe your topic — AI drafts all statements with rationales</div></div>
    ...
    {open&&(<div style={{borderRadius:12,border:`1.5px solid ${NAVY}`,background:"#fff",overflow:"hidden",boxShadow:"0 2px 8px rgba(24,48,63,.1)"}}>
      <div style={{display:"flex",alignItems:"center",gap:10,padding:".875rem 1.25rem",borderBottom:`1px solid ${BORDER}`}}>
        <div style={{width:34,height:34,borderRadius:8,background:NAVY,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M9 1L3 9h5l-1 6 7-9H9l1-5z" fill="#fff" stroke="#fff" strokeWidth=".5" strokeLinejoin="round"/></svg></div>
        <div style={{flex:1}}><div style={{fontSize:15,fontWeight:600,color:NAVY}}>Generate with AI</div><div style={{fontSize:14,color:TT}}>Describe your topic — AI drafts all statements with rationales</div></div>
      ...
      <div style={{padding:"1.125rem 1.25rem"}}>
        <textarea ref={taRef} value={desc} onChange={e=>setDesc(e.target.value)} onKeyDown={handleKey} placeholder="e.g. Common misconceptions about GDPR — include statements learners confidently get wrong" rows={3} style={{...iStyle,resize:"none",lineHeight:1.65,minHeight:90,marginBottom:".75rem"}}/>
...
export default function TrueOrFalsePage(){
  ...
  const [content,setContent]=useState(mkTF);
  ...
  const isReady=
    ...
    content.questions.every(q=>q.text.trim()&&q.rationale.trim());
  ...
  const handleDownload=()=>{const full=wrapFullHTML(outputSnippet,content.topic);const fname=(content.topic||'true-or-false').toLowerCase().replace(/[^a-z0-9]+/g,'-').slice(0,40)+'.html';downloadFile(full,fname);};
      ...
      {view==="preview"&&(
        <div style={{maxWidth:720,margin:"0 auto",padding:"2rem 1.5rem"}}>
          <div style={{marginBottom:"1.75rem"}}>
            <div style={{fontSize:14,fontWeight:600,color:RED,marginBottom:".5rem"}}>True or False</div>
          ...
          <TFPreview key={JSON.stringify(content)} data={content}/>
        </div>
      )}
      ...
      {view==="studio"&&(
        <div style={{maxWidth:720,margin:"0 auto",padding:mobile?"1rem":"1.75rem 1.5rem"}}>
          ...
          <div style={{padding:"1.25rem",background:"#fff",border:`1px solid ${BORDER}`,borderRadius:12,boxShadow:"0 1px 4px rgba(26,43,74,0.05)",marginBottom:"1.5rem"}}>
            ...
            <TFEditor content={content} onChange={setContent}/>
          </div>
          ...
          <div style={{position:"sticky",bottom:0,background:"#fff",borderTop:`1px solid ${BORDER}`,padding:".875rem 1.25rem",display:"flex",alignItems:"center",gap:"1rem",flexWrap:"wrap",marginLeft:mobile?"-1rem":"-1.5rem",marginRight:mobile?"-1rem":"-1.5rem",zIndex:50}}>
            <div style={{display:"flex",gap:"1rem",flex:1,flexWrap:"wrap"}}>
              {[
                ...
                {ok:content.questions.every(q=>q.text.trim()),label:"Statements"},
                {ok:content.questions.every(q=>q.rationale.trim()),label:"Rationales"},
